...
namespace Seacrest {
	...
	VertexBuffer* VertexBuffer::Create( void* vertices, uint32_t size, uint32_t sizeList )
	{
		switch (Renderer::GetAPI())
		{
			...
			case RendererAPI::API::Direct3D: return new Direct3DVertexBuffer(vertices, size, sizeList);
		}
	...
	IndexBuffer* IndexBuffer::Create( unsigned short* indices, unsigned short size, unsigned short sizeList)
	{
		...
		{
			case RendererAPI::API::None:     SEACREST_ASSERT( false, "RendererAPI::None is currently not supported!" );  return nullptr;
			case RendererAPI::API::Direct3D: return new Direct3DIndexBuffer( indices, size, sizeList );
		}
