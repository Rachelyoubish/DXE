...
namespace DXE {
	...
	{
		DXE_CORE_ASSERT( windowHandle, "Window handle is null!" );
	}
	...
	void D3DContext::Init()
	{
		...
		DXE_CORE_ASSERT( SUCCEEDED(result), "Failed to create DirectX device and swap chain!" );
	...
	void D3DContext::LogAdapterInfo()
	{
		...
		DXE_CORE_WARN( "DirectX Information:" );
		DXE_CORE_INFO( "Vendor:		{0}", vendor );
		DXE_CORE_INFO( "Renderer:	{0}", videoCardDescription );
		DXE_CORE_INFO( "Version:	{0}.{1}.{2}.{3}", major, minor, release, build );
	}
}